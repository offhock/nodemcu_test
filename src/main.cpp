...
void setup() {    
    ...
    mqttClient.setCallback([](char* topic, byte* payload, unsigned int length) {        
        ...
        mqttClient.publish("homeassistant/nodemcu-2/switch", message.c_str());        
    });
...
void reconnect() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(nodeName, "user", "passwd")) {
      ...
      mqttClient.subscribe("homeassistant/nodemcu-2/switch/set"); 
      // publish switch available
      mqttClient.publish("homeassistant/nodemcu-2/switch/available", "online");
    } else {
...
void loop() {
    ...
    if (!mqttClient.connected()) {
        mqttClient.publish("homeassistant/nodemcu-2/switch/available", "offline"); 
        reconnect();
    } 
    ...
    if( now - lastSent > 5000 ) {
        ...
        mqttClient.publish("homeassistant/nodemcu-2/temperature", String(temperature).c_str());
        mqttClient.publish("homeassistant/nodemcu-2/counter", String(count).c_str());
        mqttClient.publish("homeassistant/nodemcu-2/sin", String(value).c_str());
